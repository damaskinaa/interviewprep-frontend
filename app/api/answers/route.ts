...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(buildDaytonaUrl(previewUrl, "/answers/generate"), {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ error: "Answer route crashed", message: err instanceof Error ? err.message : "Unknown error" }, { status: 500 });
  }
}
