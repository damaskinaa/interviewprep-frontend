...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(buildDaytonaUrl(previewUrl, `/module/status?job_id=${encodeURIComponent(jobId)}`), {
      headers: {
        "X-App-Key": appApiKey,
    ...
    return NextResponse.json({ error: "Module status route crashed", message: err instanceof Error ? err.message : "Unknown error" }, { status: 500 });
  }
}
