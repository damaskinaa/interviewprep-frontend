...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(buildDaytonaUrl(previewUrl, "/module/run"), {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ error: "Module run route crashed", message: err instanceof Error ? err.message : "Unknown error" }, { status: 500 });
  }
}
